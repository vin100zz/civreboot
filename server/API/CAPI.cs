...
namespace OpenCivOne
{
	public class CAPI
	{
		...
		public short getch()
		{
			...
			{
				this.oCPU.AX.UInt16 = (ushort)this.oParent.Keys.Dequeue();
			}
		...
		public short fopen(string filename, string mode)
		{
			...
			short sHandle = -1;
			string sPath = $"{this.oParent.ResourcePath}{GetDOSFileName(filename.ToUpper())}";
		...
		public short _dos_open(ushort filenamePtr, ushort flags, ushort handlePtr)
		{
			...
			string sPath = $"{this.oParent.ResourcePath}{GetDOSFileName(sName.ToUpper())}";

...
namespace OpenCivOne
{
	public class MapManagement
	{
		...
		public void F0_2aea_0008_DrawVisibleMap(int playerID, int x, int y)
		{
			...
			this.parent.Var_d75e_MapViewY = this.parent.GameTools.F0_2dc4_007c_CheckValueRange(y, 0, 38);
			...
			for (int i = 0; i < this.VisibleCities.Count; i++)
			{
				...
				this.parent.DrawTools.F0_1182_0086_DrawStringWithShadow(
					...
					this.parent.GameTools.F0_2dc4_007c_CheckValueRange(cityInfo.ScreenPos.X - 8, 80, 999), cityInfo.ScreenPos.Y + 16, 11);
			}
		...
		public bool F0_2aea_03ba_DrawCell(int x, int y)
		{
			...
			else
			{
				...
				if (terrainImprovements.HasFlag(TerrainImprovementFlagsEnum.City) && this.parent.Var_dcfc == 0)
				{
					...
					int cityID = this.parent.GameTools.F0_2dc4_00ba_GetCityByLocation(x, y);
		...
		public void F0_2aea_0fb3_DrawUnitWithStatus(int playerID, int unitID, int x, int y)
		{
			...
			if (((UnitStatusEnum)unit.Status & UnitStatusEnum.SettlerBuildMask) != 0 && this.parent.GameData.Units[(int)unit.UnitType].MovementType != UnitMovementTypeEnum.Air)
			{
				string status = "R";

				if ((unit.Status & 0x40) != 0)
				{
					status = (unit.UnitType != (int)UnitTypeEnum.Settler) ? "?" : "I";
				}

	...
	public class UnitGoTo
	{
		...
		private void FindGoToPath(Unit unit, bool testVisibility)
		{
			if (unit.GoToDestination != OpenCivOneGame.InvalidPosition)
			{
				...
				UnitMovementTypeEnum movementType = this.gameData.Units[(int)unit.UnitType].MovementType;
					...
					if ((pathFlags & (0x1 << (i - 1))) != 0)
					{
						...
						if (local_6 != 0)
						{
							if (local_6 < local_8)
							{
								...
								local_a = this.parent.GameTools.F0_2dc4_0289_GetShortestDistance(unit.GoToDestination.X, unit.GoToDestination.Y,
									newX * 4 + 1, newY * 4 + 1);
							}
							else if (local_6 == local_8)
							{
								// Instruction address 0x2e31:0x08b0, size: 5
								int local_12 = this.parent.GameTools.F0_2dc4_0289_GetShortestDistance(unit.GoToDestination.X, unit.GoToDestination.Y,
									newX * 4 + 1, newY * 4 + 1);
			...
			if (newMoveDirection == -1)
			{
				...
				for (int i = 1; i < 9; i++)
				{
						...
						(!waterUnit && this.Arr_db44_LandPath[newShortX, newShortY] != 0))
					{
						...
						int distance = this.parent.GameTools.F0_2dc4_0289_GetShortestDistance(x - (newShortX * 4) - 1, y - (newShortY * 4) - 1);
								...
								if (local_6 != 0)
								{
									...
									if (local_6 < local_a)
									{
										...
										if (activeUnitID != -1)
										{
											...
											local_e = (short)this.parent.UnitManagement.F0_1866_1251_GetStackUnitValueCoefficient(unit.PlayerID, activeUnitID, 2) * 4;
										}
										else
										{
											local_e = 0;
										...
										local_e += this.parent.GameTools.F0_2dc4_0289_GetShortestDistance(this.Var_6590_DestinationX, this.Var_6592_DestinationY, newXAdjusted, newY);
									}
									...
									if (local_6 == local_a)
									{
										...
										if (activeUnitID != -1)
										{
											...
											local_1a = (short)this.parent.UnitManagement.F0_1866_1251_GetStackUnitValueCoefficient(unit.PlayerID, activeUnitID, 2) * 4;
										}
										else
										{
											local_1a = 0;
										...
										local_1a += this.parent.GameTools.F0_2dc4_0289_GetShortestDistance(this.Var_6590_DestinationX, this.Var_6592_DestinationY, newXAdjusted, newY);

...
namespace OpenCivOne
{
	public class GameLoadAndSave
	{
		...
		public GameLoadAndSave(OpenCivOneGame parent)
		{
			this.parent = parent;
		}
		...
		public void F11_0000_036a_SaveGameDialog(bool autoSave)
		{
			...
			this.parent.Var_2f9a_MenuBoxDefaultOptionIndex = this.parent.GameTools.F0_2dc4_007c_CheckValueRange(this.gameIndex, 0, 3);
			...
			{
				this.parent.DrawTools.F0_1182_00b3_DrawCenteredStringToScreen0("Game has been saved.", 125, 132, 0);
				this.parent.DrawTools.F0_1182_00b3_DrawCenteredStringToScreen0("Press key to continue.", 125, 142, 0);
				this.parent.CAPI.getch();
			}
			else
            {
				this.parent.DrawTools.F0_1182_00b3_DrawCenteredStringToScreen0("The current game was NOT saved.", 125, 132, 0);
				this.parent.DrawTools.F0_1182_00b3_DrawCenteredStringToScreen0("Press key to continue.", 125, 142, 0);
				this.parent.CAPI.getch();
			}
		...
		public bool F11_0000_083b_LoadGameData(string filename)
		{
			...
			try
			{
				...
				for (int i = 0; i < 8; i++)
				{
					for (int j = 0; j < 16; j++)
					{
						this.parent.GameData.Players[i].ContinentPolicies[j].Position.X = (sbyte)ReadUInt8(reader);
					}
				}
				...
				for (int i = 0; i < 8; i++)
				{
					for (int j = 0; j < 16; j++)
					{
						this.parent.GameData.Players[i].ContinentPolicies[j].Position.Y = (sbyte)ReadUInt8(reader);
					}
				}
			...
			catch (Exception ex)
			{
				...
				this.parent.Segment_1238.F0_1238_001e_ShowDialog(ex.Message, 100, 80);
		...
		public bool F11_0000_08f6_SaveGameData(string filename)
		{
			...
			{
				this.parent.MenuBoxDialog.F0_2d05_0031_ShowMenuBox(ex.Message, 4, 64, true, false, true);
